...
if [ -h "$dpath1" ];then
  tmplibpath1=`readlink $dpath1` #bin--->usr/bin
[ ! -d "$dpath$tmplibpath1" ] &&  mkdir -p $dpath$tmplibpath1 #/mnt/sysroot/usr/bin
fi

if [ -h "$dpath2" ];then
  tmplibpath1=`readlink $dpath2` #sbin--->usr/sbin
[ ! -d "$dpath$tmplibpath2" ] &&  mkdir -p $dpath$tmplibpath2 #/mnt/sysroot/usr/sbin
...
if which $1 &> /dev/null;then
 ...
 if [ -f "$dpath$libline"  -o -h "$dpath$libline" ];then  #/mnt/sysroot/lib64/libdl.so.2 exist or not exist
   ...
   if [ -h "$libline" ];then
     ...
     cp -a $dpath$dpathlib3  $dpath1$tmplibpath
     return 0
